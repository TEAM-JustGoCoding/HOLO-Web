<?php
  ...
  $rquery = "SELECT delivery_reply.id, delivery_reply.delivery_id, delivery_reply.content, delivery_reply.date,
            user.nick_name, user.id AS `user_id`, user.uid AS `mail`
            FROM `delivery_reply` left join `user`
            on delivery_reply.user_id = user.id
