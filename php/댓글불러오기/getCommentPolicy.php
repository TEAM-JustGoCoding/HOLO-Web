<?php
  ...
  $rquery = "SELECT policy_reply.id, policy_reply.policy_id, policy_reply.content, policy_reply.date,
            user.nick_name, user.id AS `user_id`, user.uid AS `mail`
            FROM `policy_reply` left join `user`
            on policy_reply.user_id = user.id WHERE policy_reply.policy_id = $postId;";
