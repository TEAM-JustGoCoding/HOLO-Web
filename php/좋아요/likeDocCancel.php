<?php
  ...
  $dquery = "DELETE FROM `document_like` WHERE `document_like`.`document_id` = '$postId' AND `document_like`.`user_id` = '$userId';";
  $result = mysqli_query($conn, $dquery);

  //좋아요 개수 산출
  $rquery = "SELECT * from `document_like` WHERE `document_like`.`document_id` = '$postId';";
  ...
  $uquery = "UPDATE `document_post` SET `like` = '$count' WHERE id = '$postId';";
