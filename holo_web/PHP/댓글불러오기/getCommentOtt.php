<?php
  ...
  $rquery = "SELECT ott_reply.id, ott_reply.ott_id, ott_reply.content, ott_reply.date,
            user.nick_name, user.id AS `user_id`, user.uid AS `mail`
            FROM `ott_reply` left join `user`
            on ott_reply.user_id = user.id WHERE ott_reply.ott_id = $postId;";
